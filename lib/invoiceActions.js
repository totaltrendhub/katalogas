...
async function generateInvoicePdfBytes(invoice, settings) {
  ...
  const totalNet =
    invoice.total_without_vat != null
      ...
      : qty * unitPrice;
  const vatRate =
    invoice.vat_rate != null ? Number(invoice.vat_rate) : 0;
  const vatAmount =
  ...
  if (sellerCode) sellerLines.push(`Įmonės kodas: ${sellerCode}`);
  ...
  if (sellerIban) sellerLines.push(`Sąskaita: ${sellerIban}`);
  ...
  const labelNr = "Nr.:";
  const labelNrWidth = fontRegular.widthOfTextAtSize(labelNr, fontSizeNormal);

  page.drawText(labelNr, {
    x: titleX + 140 - labelNrWidth,
    y: titleY,
    size: fontSizeNormal,
    font: fontRegular,
  ...
  const numberWidth = fontBold.widthOfTextAtSize(number, fontSizeNormal);
  page.drawText(number, {
    x: titleX + 140 - numberWidth,
    y: titleY - 14,
    size: fontSizeNormal,
    font: fontBold,
    color: rgb(0.15, 0.15, 0.15),
  ...
  const drawHeaderCell = (text, x, align = "left") => {
    ...
    const widthLabel = fontBold.widthOfTextAtSize(label, fontSizeNormal);
    let drawX = x;
