...
/**
 * IŠTRINTI REKLAMĄ (IR ATLAISVINTI SLOTĄ, NUTRAUKTI RYŠĮ SU SĄSKAITOMIS)
 */
export async function deleteAdAction(formData) {
  ...
  // admin check – tik adminas gali trinti
  const { data: profile, error: profileError } = await supabase
    .from("profiles")
    .select("is_admin")
    .eq("id", user.id)
  ...
  if (profileError || !profile?.is_admin) {
    console.error(
      "deleteAdAction profile/admin ERROR:",
      profileError
    );
    redirect("/dashboard/ads?error=delete_failed");
  }

  // pasiimam reklamą ir jos slotą
  const { data: ad, error: adError } = await supabase
    .from("ads")
    .select("id, slot_id")
    .eq("id", adId)
  ...
  // 1) visoms sąskaitoms, kurios rodo į šią reklamą, nuimam ad_id,
  // kad nebeliktų FK blokuojančio trynimo
  ...
  if (invoicesUpdateError) {
    // čia log'as – bet vartotojui vis tiek bandome tęsti,
    // nes blogiausiu atveju DB neleis ištrinti ir pamatysim klaidą žemiau
    console.error(
      "deleteAdAction invoices update ERROR:",
      invoicesUpdateError
    );
  }

  // 2) jei slotas buvo pririštas – atlaisvinam jį
  if (ad.slot_id) {
  ...
  // 3) galiausiai trinam pačią reklamą
